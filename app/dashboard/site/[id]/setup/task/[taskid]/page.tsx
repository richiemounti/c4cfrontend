// app/dashboard/site/[id]/setup/task/[taskid]/page.tsx
'use client';
...
export default function SiteSetupTaskPage() {
  const router = useRouter();
  const { id: siteId, taskid: taskId } = useParams() as { id: string; taskid: string };
  ...
  // Fetch site setup data when component mounts
  useEffect(() => {
    const fetchSetupData = async () => {
      try {
        ...
        if (!response.isInitialized) {
          // Redirect to site page if setup is not initialized
  ...
  const handleNavigate = (index: number) => {
    if (setupData && index >= 0 && index < setupData.tasks.length) {
      ...
      router.push(`/dashboard/site/${siteId}/setup/task/${nextTask._id}`);
    }
  };
