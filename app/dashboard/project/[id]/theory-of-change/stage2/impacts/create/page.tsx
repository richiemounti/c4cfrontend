// app/dashboard/project/[id]/theory-of-change/stage2/impacts/create/page.tsx
...
import { defineOutcome } from '@/lib/api/theoryOfChange';
...
export default function CreateImpactPage() {
  ...
  const getErrorMessage = (error: any): string => {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 409:
          return 'An outcome with the same content already exists for this stakeholder group.';
        case 422:
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Get or initialize Stage 2 — pass siteId so we get the site-level stage when in site context
        ...
        if (stage2) {
          ...
          console.error("Stage 2 not found");
          toast({
            title: "Error",
            description: "Stage 2 not found. Please contact support.",
            variant: "destructive",
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (data: CreateImpactData) => {
    try {
      ...
      toast({
        title: "Creating Outcome",
        description: "Please wait while we create your outcome...",
      });
      
      await defineOutcome(impactData);
        ...
        <div className="bg-white px-8 py-6 border-b border-sky">
          ...
          <div>
            <h1 className="text-2xl font-medium text-stratosphere">
              Define New Social Outcome {siteId && <span className="text-gray-500">(Site Level)</span>}
            </h1>
