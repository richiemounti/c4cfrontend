          ...
          {rawOptions.map((opt: any, index: number) => {
            ...
            return (
              <div key={index} className="rounded-lg border border-stone-200 overflow-hidden">
                <div className="flex items-center space-x-2 px-3 py-2 hover:bg-stone-50 transition-colors">
                ...
                {descriptor !== undefined && descriptor !== null && isSelected && (
                  <div className="px-3 pb-3 pt-1 bg-neutral-50 border-t border-neutral-100">
                    {descriptor && (
                      <p className="text-xs text-neutral-700 mb-1.5">{descriptor}</p>
                    )}
                    <Input
        ...
        <div className="space-y-3">
          ...
          {selectDescriptor !== undefined && selectDescriptor !== null && value && (
            <div className="px-3 pb-3 pt-2 bg-neutral-50 border border-neutral-200 rounded-lg">
              {selectDescriptor && (
                <p className="text-xs text-neutral-700 mb-1.5">{selectDescriptor}</p>
              )}
              <Input
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 w-14 flex-shrink-0 rounded-lg p-1 transition-colors ${
                        selected ? 'bg-neutral-100' : 'hover:bg-stone-50'
                      }`}
                    >
                      ...
                      <span className="text-xs text-center leading-tight w-14 break-words text-neutral-500">
                        {opt.label}
                      </span>
                    </button>
                  );
          ...
          <div className="flex flex-wrap gap-2">
            {points.map(pt => (
              <Button
                ...
                size="sm"
                onClick={() => onChange(pt)}
                className={`w-10 h-10 ${value === pt ? 'bg-neutral-500 border-neutral-500' : ''}`}
              >
                {pt}
              </Button>
            ))}
      ...
      return (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="p-3 text-left border-b border-stone-200 w-1/3" />
                {mc.columns.map((col, i) => (
                  <th key={i} className="p-3 text-center text-sm font-medium text-neutral-700 border-b border-stone-200">
                    {col.label}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {mc.rows.map((row, ri) => (
                <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-stone-50'}>
                  <td className="p-3 text-sm font-medium text-ink border-r border-stone-100">
                    {row.label}
                    {row.description && (
                      <p className="text-xs text-neutral-400 mt-0.5">{row.description}</p>
                    )}
                  </td>
    ...
    default:
      return (
        <div className="p-4 bg-stone-50 rounded-lg">
          <p className="text-sm text-neutral-500">Question type "{questionType}" not yet supported in preview</p>
        </div>
      );
  }
};
  ...
  if (showThankYou) {
    return (
      <div className={`${isModal ? 'p-6' : 'max-w-2xl mx-auto'}`}>
        <Card className="text-center bg-gradient-to-br from-coral-50 to-neutral-50">
    ...
    <div className={`${isModal ? 'p-6' : 'max-w-2xl mx-auto'}`}>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-neutral-500">Survey Progress</span>
      ...
      {/* Survey Header */}
      <Card className="mb-6 bg-gradient-to-br from-white to-ink-50/30">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-semibold text-ink-900">
            {structure.survey.title}
          </CardTitle>
          {structure.survey.description && (
            <p className="text-neutral-500 mt-2">{structure.survey.description}</p>
          )}
        </CardHeader>
      </Card>
      ...
      {currentSection && currentSection._id !== 'no-section' && (
        <Card className="mb-6 bg-gradient-to-r from-petrol-50 to-sage-50">
          <CardHeader>
            <CardTitle className="text-lg text-petrol-900 flex items-center gap-2">
              <FileText className="h-5 w-5" />
              {currentSection.title}
            </CardTitle>
            {currentSection.description && (
              <p className="text-petrol-600 text-sm">{currentSection.description}</p>
            )}
          </CardHeader>
        </Card>
      )}
          ...
          <CardContent className="p-8">
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="flex items-center justify-center w-8 h-8 bg-neutral-50 rounded-full text-sm font-medium text-neutral-500 mt-1">
                  {questionsAnswered + 1}
                </div>
