...
const ViewRisksModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col bg-white border-neutral-200">
        <DialogHeader>
          <DialogTitle className="text-xl text-ink flex items-center">
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-neutral-500">Loading risks...</span>
            </div>
          )}
          ...
          {!loading && !error && risks.length > 0 && (
            <div className="space-y-3">
              {risks.map((risk) => (
                <div
                  key={risk._id}
                  className="p-4 border border-stone-200 rounded-lg hover:border-neutral-300 transition-colors cursor-pointer"
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="px-2 py-1 rounded-full text-xs bg-stone-100 text-neutral-700">
                          {risk.riskType}
                        </span>
                      </div>
        ...
        {selectedRisk && (
          <Dialog open={!!selectedRisk} onOpenChange={() => setSelectedRisk(null)}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white">
              <DialogHeader>
                <DialogTitle className="text-xl text-ink flex items-center justify-between">
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-neutral-700 mb-1">Risk Type & Score</h4>
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-600">{selectedRisk.riskDescription}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-sm font-medium text-neutral-700 mb-1">Probability</h4>
                    <p className="text-sm text-neutral-600 capitalize">{selectedRisk.probability.replace('_', ' ')}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-neutral-700 mb-1">Consequences</h4>
                    <p className="text-sm text-neutral-600 capitalize">{selectedRisk.consequences}</p>
                  </div>
                </div>

                <div>
                  <h4 className="text-sm font-medium text-neutral-700 mb-1">Mitigation Strategy</h4>
                  <p className="text-sm text-neutral-600">{selectedRisk.mitigationStrategy}</p>
                </div>

                <div>
                  <h4 className="text-sm font-medium text-neutral-700 mb-1">Owner</h4>
                  <p className="text-sm text-neutral-600">{selectedRisk.owner.name}</p>
                </div>

                {selectedRisk.projectSite && (
                  <div>
                    <h4 className="text-sm font-medium text-neutral-700 mb-1">Project Site</h4>
                    <p className="text-sm text-neutral-600">{selectedRisk.projectSite.name}</p>
                  </div>
                )}
              </div>
            </DialogContent>
