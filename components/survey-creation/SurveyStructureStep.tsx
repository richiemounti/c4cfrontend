...
function QuestionItem({ 
  ...
  return (
    <div className="border border-stone-500/20 rounded-lg p-4 bg-white hover:shadow-sm transition-shadow">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <h4 className="font-medium text-ink-900 text-sm line-clamp-2">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Badge variant="outline" className="text-xs border-stone-500/30">
                  {questionItem.question?.type}
                </Badge>
                {questionItem.question?.theme && (
                  <Badge variant="secondary" className="text-xs bg-ink-50 text-ink-900">
                ...
                {questionItem.question?.isBespoke && questionItem.question.bespokeMetadata && (
                  <div className="flex flex-col gap-0.5">
                    ...
                    {questionItem.question.bespokeMetadata.status === 'pending' && (
                      <Badge className="text-xs bg-gold-100 text-gold-900 border-gold-300">
                        <Wand2 className="h-3 w-3 mr-1" />
                        Pending
                      </Badge>
                    )}
                    {questionItem.question.bespokeMetadata.status === 'approved' && (
                      <Badge className="text-xs bg-sage-100 text-sage-900 border-sage-300">
                        <Wand2 className="h-3 w-3 mr-1" />
                        Approved
                      </Badge>
                    )}
                    {questionItem.question.bespokeMetadata.status === 'rejected' && (
                      <Badge className="text-xs bg-coral-100 text-coral-900 border-coral-300">
                        <Wand2 className="h-3 w-3 mr-1" />
                        Rejected
                      </Badge>
                    )}
                    {questionItem.question.bespokeMetadata.status === 'elevated' && (
                      <Badge className="text-xs bg-neutral-100 text-neutral-900 border-neutral-300">
                        <Wand2 className="h-3 w-3 mr-1" />
                        Elevated
                      </Badge>
                    )}
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                <SelectTrigger className="w-[160px] h-8 text-xs border-stone-500/30">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Button
                ...
                className="h-8 text-xs border-stone-500/30 hover:border-petrol-500"
              >
                {questionItem.required ? (
                  <>
                  ...
                  <>
                    <Circle className="h-3 w-3 mr-1 text-neutral-500" />
                    Optional
                  </>
                )}
              </Button>
    ...
    <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-white rounded-lg p-4 border border-stone-500/10">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-semibold text-ink-900 text-sm">Sections</h4>
                  </div>
                ...
                <div className="bg-white rounded-lg p-4 border border-stone-500/10">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-semibold text-ink-900 text-sm">Required Questions</h4>
                  </div>
                  <p className="text-xs text-neutral-500">
                ...
                <div className="bg-white rounded-lg p-4 border border-stone-500/10">
                  <div className="flex items-center gap-2 mb-2">
                    <Move3D className="h-4 w-4 text-sage-500" />
                    <h4 className="font-semibold text-ink-900 text-sm">Organization</h4>
                  </div>
                  <p className="text-xs text-neutral-500">
      ...
      <Card className="bg-white border-stone-500/20">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <CardTitle className="flex items-center gap-2">
                <Layers className="h-5 w-5 text-petrol-500" />
                Survey Structure
              </CardTitle>
        ...
        <CardContent className="space-y-6">
          {/* Quick Info Alert */}
          <Alert className="border-petrol-500/30 bg-petrol-50">
            <AlertCircle className="h-4 w-4 text-petrol-500" />
            <AlertDescription className="text-sm text-neutral-500">
              ...
              Survey flow: <strong className="text-ink-900">Sections (in order) → Unassigned Questions</strong>
            </AlertDescription>
          </Alert>
          ...
          {sections.map(section => (
            ...
            >
              <div className="border border-petrol-200 rounded-lg">
                <CollapsibleTrigger asChild>
                  <div className="flex items-center justify-between p-4 bg-petrol-50 rounded-t-lg hover:bg-petrol-100 cursor-pointer transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs border-petrol-500/30 text-petrol-500">
                      ...
                      <Button
                        ...
                        className="text-gold-500 hover:text-gold-700 hover:bg-gold-50"
                      >
          ...
          <div className="border border-stone-500/20 rounded-lg">
            <div className="p-4 bg-stone-50 rounded-t-lg">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-medium text-ink-900">
                    Unassigned Questions
            ...
            <div className="p-4 space-y-3">
              {unassignedQuestions.length === 0 ? (
                <div className="text-center py-8 text-neutral-500">
                  <CheckCircle className="h-8 w-8 mx-auto mb-2 text-sage-500" />
                  <p className="text-sm font-medium text-ink-900">All questions organized!</p>
                  <p className="text-xs text-neutral-500 mt-1">All questions have been assigned to sections</p>
                </div>
              ) : (
      ...
      <div className="flex justify-between">
        <Button variant="outline" onClick={onBack} className="border-stone-500/30">
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back: Survey Details
        </Button>
        <Button onClick={onNext} className="bg-coral-500 hover:bg-coral-600 text-white">
      ...
      <Dialog open={newSectionDialog} onOpenChange={setNewSectionDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setNewSectionDialog(false)} className="border-stone-500/30">
              Cancel
            </Button>
            <Button 
