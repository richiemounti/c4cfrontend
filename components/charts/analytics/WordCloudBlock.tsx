  ...
  return (
    <div className="space-y-4">
      ...
      {stats.topWords.length > 0 && (
        <div>
          <p className="text-xs font-medium text-neutral-500 mb-1.5">Frequently mentioned words</p>
          <div className="flex flex-wrap gap-1.5">
            {stats.topWords.slice(0, 12).map(([word, count]) => (
              <span
                key={word}
                className="px-2 py-1 rounded-full bg-neutral-50 text-ink"
      ...
      {sample.length > 0 && (
        <div>
          <p className="text-xs font-medium text-neutral-500 mb-1.5">Sample responses</p>
          <ul className="space-y-1.5">
            {sample.map((quote, i) => (
              <li key={i} className="text-sm text-ink italic border-l-2 border-neutral-200 pl-2">
                &ldquo;{quote}&rdquo;
              </li>
            ))}
          </ul>
