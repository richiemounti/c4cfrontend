  ...
  const renderInput = () => {
    switch (question.type) {
      case 'text':
        return (
          <input 
            ...
            className="w-full px-3 py-2 border border-stone-300 rounded-md focus:ring-2 focus:ring-coral-500 focus:border-coral-500"
          />
        );
        ...
        return (
          <div className="space-y-3">
            {(question.options || []).map((opt: any, index: number) => (
              <div key={index} className="rounded-lg border border-stone-200 overflow-hidden">
                <label className="flex items-center space-x-3 cursor-pointer px-3 py-2 hover:bg-stone-50 transition-colors">
                  <input
                    type="radio"
                    ...
                    onChange={(e) => handleChange(e.target.value)}
                    className="h-4 w-4 text-coral-600 focus:ring-coral-500 border-stone-300"
                  />
                ...
                {opt.descriptor !== undefined && opt.descriptor !== null && value === opt.value && (
                  <div className="px-3 pb-3 pt-1 bg-neutral-50 border-t border-neutral-100">
                    {opt.descriptor && (
                      <p className="text-xs text-neutral-700 mb-1.5">{opt.descriptor}</p>
                    )}
                    <input
                      type="text"
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-ink-200 rounded-md focus:ring-2 focus:ring-coral-500 focus:border-coral-500 bg-white"
                    />
                  </div>
                )}
              </div>
                ...
                <div key={index} className="rounded-lg border border-stone-200 overflow-hidden">
                  <label className="flex items-center space-x-3 cursor-pointer px-3 py-2 hover:bg-stone-50 transition-colors">
                    <input
                      ...
                      className="h-4 w-4 text-coral-600 focus:ring-coral-500 border-stone-300 rounded"
                    />
                    <span className="text-ink">{opt.label}</span>
                  </label>
        ...
        return (
          <div className="space-y-3">
            ...
            {hasDropdownDescriptor && (
              <div className="px-3 pb-3 pt-2 bg-neutral-50 border border-neutral-200 rounded-lg">
                {selectedDropdownOption.descriptor && (
                  <p className="text-xs text-neutral-700 mb-1.5">{selectedDropdownOption.descriptor}</p>
                )}
                <input
                  type="text"
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="text-center">
              <span className="inline-flex items-center justify-center w-12 h-8 bg-coral-100 text-coral-800 rounded-md font-medium">
                {currentValue}
              </span>
            </div>
            ...
            {scaleOptions.length > 0 && (
              <div className="space-y-3 mt-6">
                <h4 className="text-sm font-medium text-neutral-700">Rating Scale:</h4>
                      ...
                      <div className="flex items-start gap-3">
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-medium ${
                          isSelected 
                            ...
                            : 'border-stone-300 text-neutral-600'
                        }`}>
                          {option.value}
                        </div>
            ...
            <div className="flex items-center justify-center w-full">
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-stone-300 border-dashed rounded-lg cursor-pointer bg-stone-50 hover:bg-stone-100">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  <Paperclip className="w-8 h-8 mb-2 text-neutral-400" />
      ...
      case 'location':
        return (
          <div className="border-2 border-dashed border-stone-300 rounded-lg p-8 text-center bg-stone-50">
            ...
            <p className="text-sm text-neutral-600">Location selector would appear here</p>
            <p className="text-xs text-neutral-500 mt-1">GPS coordinates: {value || 'Not selected'}</p>
          </div>
        );
          ...
          <div className="space-y-4">
            ...
            {matrixColumns.some((col: any) => col.description) && (
              <div className="bg-stone-50 p-4 rounded-lg">
                <h4 className="text-sm font-medium text-neutral-700 mb-2">Answer Options:</h4>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border border-stone-300 rounded-lg">
                <thead>
                  <tr className="bg-stone-50">
                    <th className="border border-stone-300 px-4 py-3 text-left text-sm font-medium text-neutral-700 min-w-[200px]">
                      Questions
                    </th>
                    {matrixColumns.map((column: any, index: number) => (
                      <th key={index} className="border border-stone-300 px-3 py-3 text-center text-sm font-medium text-neutral-700 min-w-[120px]">
                ...
                <tbody>
                  {matrixRows.map((row: any, qIndex: number) => (
                    <tr key={qIndex} className="bg-white hover:bg-stone-50">
                      <td className="border border-stone-300 px-4 py-3 text-sm text-ink">
                      ...
                      {matrixColumns.map((column: any, oIndex: number) => (
                        <td key={oIndex} className="border border-stone-300 px-3 py-3 text-center">
                          <input
                            type="radio"
                            name={`matrix-${qIndex}`}
                            value={column.value}
      ...
      default:
        return (
          <div className="border-2 border-dashed border-stone-300 rounded-lg p-8 text-center bg-stone-50">
            <div className="w-8 h-8 bg-stone-200 rounded-lg mx-auto mb-2 flex items-center justify-center">
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            {question.description && (
              <p className="text-neutral-600 text-sm mt-1">{question.description}</p>
            )}
            
            {/* NEW: Demographics indicator */}
            {question.isStandardDemographic && (
              <div className="flex items-center gap-2 mt-2">
                <Badge variant="outline" className="text-xs bg-gold-50 text-gold-800 border-gold-200">
                ...
                {question.isGlobalStandard && (
                  <Badge variant="outline" className="text-xs bg-petrol-50 text-petrol-700 border-petrol-200">
          ...
          <div className="flex items-center justify-center w-8 h-8 bg-petrol-100 rounded-lg ml-4">
            {getQuestionTypeIcon(question.type)}
          </div>
        </div>
      </div>
